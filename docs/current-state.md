...
**Updated By**: Phase 3.4.1.2B delete-verification + activity timeline (AI agent)

> **Phase 3.4.1.2B (2026-08-04)** — Delete-contract verification + activity timeline + admin purge + Party track entry (frontend + one additive admin DB function; playback + UI preserved; Provider). **Delete "regression" root-caused to NONE:** `playlist_delete` is a correct owner-checked soft-delete; every "deleted" playlist has `deleted_at` set (verified in DB, incl. the latest-APK `prueba 2`); `can_view_playlist`=false for deleted + read paths filter `deleted_at is null`, so deleted playlists never appear in-app. Rows remaining in the table is the soft-delete contract, not a bug. **Decision: keep soft-delete.** Added DB acceptance test + Hive removal/restart tests + read-path verification. Added an **admin-only** `private.purge_soft_deleted_playlist` (EXECUTE revoked; refuses live playlists) and used it once to hard-purge the 5 confirmed soft-deleted test rows (0 orphans; `prueba 3` untouched). **Activity timeline (3.4.1.2A):** the sheet showed only "created" because the client fetched `limit(1)`; the DB already logs the full history (verified). Replaced with a paginated, realtime, newest-first timeline (≥30 + load-more, dedupe by id, actor avatar/username, icons, presentation-only 5-min grouping; created never suppresses later events). **Party track entry:** "Start a Party with this song" added to track menus via the shared seam, gated by `AppConfig.partyEnabled` (default OFF → hidden, consistent with Library). +~20 tests (272 pass); analyze clean; no new advisor. Cloud Likes NOT started. PR pending.

> **Phase 3.4.1.2 (2026-08-04)** — Follow notifications + avatars + notification deep-nav + activity polish + cloud playlist integrity audit (frontend + one additive DB migration redefining `playlist_set_follow` + the notification emitter; no schema/table change; no `paax-api`/Railway change; playback + UI preserved; Provider). **A)** Following a viewable playlist emits exactly one `playlist_followed` to the owner from inside the follow RPC (deduped, `FOUND`-gated so idempotent re-follow doesn't re-notify, never self, **unfollow silent** by product decision); follow does NOT change `updated_at`/`last_modified_*`. **B)** Emitter payload carries `actor_avatar`; one canonical `ActorAvatar` widget (circular, error/placeholder fallback, "Deleted user" for missing actor, never a UUID). **C)** Tapping a playlist notification marks read + opens the correct Playlist Detail under RLS (deleted/private/blocked → "no longer available", no crash/loop); Detail falls back to the passed entity for not-in-library targets (e.g. pending invites). **E)** Centralized `PlaylistActivityPresentation` (distinct icons, friendlier public/private copy, bounded track summary, a11y). **F)** Playlist follower count is now realtime for viewers (unguarded `followers` event carrying the absolute count; no delta double-apply; follow-state never inferred from the count). **G/I)** Read-only integrity audit: 3 production playlists, perfect integrity (0 orphans, 0 counter mismatches) — 1 valid live owned + 2 legitimately owner-soft-deleted; **no disposable artifacts → no cleanup**. Two accounts exist: active `iamleizu@gmail.com` owns all 3; `uziel.sando@hotmail.com` never signed in and owns none. +25 tests (257 pass); analyze clean; follow lifecycle verified in prod (rolled back). Cloud Likes NOT started. PR pending.

> **Phase 3.4.1.1 (2026-08-03)** — Cloud playlist stabilization + invitation notifications + privacy management + Party entry scaffold (frontend + additive DB migration extending the pre-existing `notifications` table; no `paax-api`/Railway change; playback + UI preserved; Provider). **A)** Edit-Order first row no longer hides under the fixed bar (padding composed from the real bar height via a pure, unit-tested helper). **B)** Delete is now **RPC-first with rollback** (owner-only `playlist_delete`, throws before any local change → fixes "deleted locally but still in Supabase" / local-only deletion); role-correct overflow (owner=Delete, collaborator=Leave, follower=Remove-from-library, invitee=Decline) — RPC/RLS authoritative, UI is convenience. **C)** "Last modified…" is tappable for **every** cloud playlist (on-demand fetch + synthesized "created" event; actor username, never a UUID). **D)** Owner "Edit privacy" (Private/Public) via version-checked RPC, grouped `visibility_changed` activity, conflict rollback; visibility ⊥ collaboration. **E)** **Invitation notifications**: `notifications` extended (`actor_user_id, entity_type, entity_id, acted_at, dedupe_key, deleted_at` + partial-unique dedupe + realtime); trusted `private.emit_playlist_notification` (revoked from clients) fires inside the collab RPCs (invite/accept/decline/leave/remove/transfer) to the right recipient, deduped; Flutter inbox layer + **Home bell** (live badge, hidden@0, "99+") + **Notifications screen** (Today/Earlier, unread-distinct, pull-to-refresh, realtime, Accept/Decline inline, mark-read, account-switch safe). **F)** Bell sits left of profile. **G)** "+" opens Create-playlist / **Start-a-Party** action sheet — Party is an **entry scaffold only** behind `AppConfig.partyEnabled` (default **OFF**): informational prep sheet, creates nothing, no Party backend. Notification forgery closed (no client INSERT policy; emitter revoked; verified with rolled-back production disposable-user tests). +32 tests. **Multi-device realtime + audible playback = on-device QA.** Cloud likes/hidden/session NOT started. PR pending.

> **Phase 3.4.1 (2026-08-02)** — Cloud Playlists + collaboration/ownership/activity/following/cross-device sync. Playlists are now authoritative in Supabase (Hive = offline cache/mirror/journal); UI/playback/collage/device-local pins preserved. Additive DB migrations only (playlist tables were empty; no data risk; no `paax-api`/Railway change). Adds `playlists` versioning/soft-delete/last-modified, invitation-lifecycle collaborators, `playlist_activity`, `user_blocks`; 14 permission-/version-checked SECURITY DEFINER RPCs (incl. ownership transfer + account-deletion succession via an `auth.users` delete trigger + a blocking seam); a Flutter cloud-sync layer (permissions/activity/remote/offline-journal/realtime/migration/repository); role-dependent Playlist Detail UI (contributors · last-modified header with tappable activity sheet; Follow/Add-to-playlist for non-members, Pin→overflow; owner Manage-collaborators + Transfer-ownership; invitee Accept/Decline); LibraryController integration (new playlists cloud-backed, push-on-mutation, idempotent migration+re-key, hydration of owned/collaborating/followed). Verified via JWT-claim + two-account production scenario (self-cleaning); 40+ new Flutter tests. **Multi-device realtime + audible playback = on-device QA.** Cloud likes/hidden/session NOT started. PR pending.

> **Phase 3.3.6 (2026-08-01)** — playlist + Library stabilization before Phase 3.4 cloud sync (frontend only; no backend/Supabase/Railway change; playback unchanged; UI preserved). Playlist Detail metadata is now 3 lines (title / owner + accepted collaborators / `Visibility · N songs · duration`) — no "Collaborative" label, collaboration shown by the participant line, visibility independent. Cloud-ready playlist model (owner, collaborators, visibility, isCollaborative, explicit normalized track positions) + repo seam `updatePlaylistTrackPositions` (local-only). Reorder is staged and committed only on Save (cancel/back retains committed order; survives restart). Cover collage fixed to fill its box as an even 2×2 square (was clipped by a size/parent mismatch). Library blank gap fixed by measuring the header and padding all tabs via one shared helper (was hardcoded `safeTop + 230`). Pinned playlists are device-local AND per-account (no cross-account leak). Idempotent backward-compat migration. PR #76. Phase 3.4 NOT started.

> **Phase 3.3.5 (2026-07-31)** — real-time global follower counts (frontend + one DB migration; no `paax-api` change, no redeploy). The follower pill previously combined a stale 24h-cached API baseline with the current user's local follow delta (`reconcileFollowerCount`), so cross-user counts were wrong (B saw 1 not 2; unfollow showed 0 while A still followed). Now a new **`FollowerCountService`** renders the AUTHORITATIVE `artists.platform_followers_count` via a direct read + a **Supabase Realtime** UPDATE subscription on the public `artists` row (aggregate only — RLS-safe; `user_followed_artists` is own-rows-only so it can't be the source). Cached API only *seeds* (never overwrites a realtime/server value); optimistic +1/-1 reconciles via realtime with a raced-refresh guard (no 2→1→2). Ref-counted channel per artist, disposed on leave, refreshed on resume, keyed by canonical UUID, isolated per account. Migration adds `public.artists` to the realtime publication. DB counter verified authoritative (controlled 0→1→2→1→0 sim). **Multi-device realtime delivery is on-device QA.** PR #75.

> **Phase 3.3.4 (2026-07-30)** — frontend-only fix (no backend/DB change, no redeploy). Per-track credits now resolve for **every** album, not just played ones. 3.3.3's overlay ran inline with a 2.5s timeout, but the normalized 16-track album response is ~3.2s even warm → it timed out for unplayed albums (att., DeBÍ TiRAR MáS FOToS) and fell back to the album-primary artist. The overlay now runs **progressively off the album-open path** (`enrichAlbumCredits`, 12s bound, one batched call, videoId preserved), with a **max-wins** guard (never downgrades richer play-queue credits) and a **coverage-aware retry** in the repository (retries only a partially-ingested album, never a not-in-catalog one). att. → "Young Miko, Feid"/"Young Miko, Dei V" etc.; SOMA unchanged. Data was already complete (verified read-only); on-device visual QA recommended. PR #74 → `1a08e57`.
