...
- **Onboarding completion is RPC-only** (Phase 3.2A): `profiles.onboarding_completed` cannot be set by a client `ProfileRepository.updateOwn` (it is excluded from the writable whitelist). It is flipped **only** by `public.complete_artist_onboarding(p_artist_ids uuid[])` — a **SECURITY DEFINER** function (`set search_path=''`, `EXECUTE` granted to `authenticated` only) that requires `auth.uid()` (else `42501`), validates ≥5 unique existing artists (dedups input; `<5` → `22023`; non-existent → `23503`), inserts idempotent `user_followed_artists` follows, and atomically flips the flag, returning `jsonb {onboarding_completed, followed_count}`. See [`../features/onboarding.md`](../features/onboarding.md), [`database-schema.md`](database-schema.md).
