...
| Flutter / paax-api integration | ❌ **Not connected** — Hive + `/v2/*` remain the live path |
...
| Table | Key points |
|-------|-----------|
| `profiles` | 1:1 `auth.users` (cascade). `username` unique `^[a-z0-9_.]{3,30}$`. **Private fields** (birth_date, gender_identity, location, coords) never publicly exposed. **Privileged fields** (`app_role` ∈ user/moderator/admin/owner; `subscription_tier/status/expires_at` — *cache only*, authoritative = `user_subscriptions`) are guarded by the `protect_profiles_privileged_columns` trigger: clients can never change them. |
...
`user_liked_tracks`, `user_saved_albums`, `user_followed_artists`, `user_followed_genres` — composite-PK relation tables; insert/delete bumps the corresponding catalog counter via `private.bump_*` triggers.
...
*Last updated: 2026-07-16 (Phase 1 schema deployed — ADR-009)*
