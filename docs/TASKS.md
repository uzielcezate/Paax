...
- [ ] **TASK-202** — Real **per-user auth + cloud library sync** to replace the demo stub. *(FR-002)* — **Phase 1 foundation (ADR-009)** + **Phase 2 backend catalog (ADR-010)** + **Phase 3.1 auth** complete. **Phase 3.2A complete (2026-07-17, ADR-011)**: real 5-artist onboarding (`complete_artist_onboarding` RPC), real Supabase profile + avatar upload, offline-first cloud library sync (liked/saved-albums/followed-artists/hidden-tracks; `user_hidden_tracks` table). **Phase 3.2B complete (2026-07-17, ADR-012)**: **followed genres** (3.2.4) on the same offline-first pipeline (`user_followed_genres`, no migration) with a Follow pill on the existing `GenreResultsScreen`, and a **personalized Home** (3.2.5) rebuilt from real Supabase catalog sections (`HomeRepository`/`HomeController`, per-user cache). **Remaining/next**: advanced recommendations, playlists cloud migration, qualified-play listening history, offline downloads — all deferred/future.
