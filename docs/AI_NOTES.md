...
## Phase 3.4.15 — a queued op can carry INTENT, not just UUIDs (2026-08-24)

**Identity was resolved BEFORE the journaling wrapper.** `addTracks`/`saveOrder`
called `_resolveOrThrow` first, and `_online(journalOnNetworkError:)` — the only
thing that ever enqueued an op — ran after it. So a track with no catalog UUID
could never be queued: resolution raised `localIntegrity`, which is terminal and
non-network, the optimistic row was rolled back, and the sheet said "that song
isn't available in Paax yet". For an un-ingested Artist Top Track added OFFLINE
that outcome was guaranteed — its UUID cannot exist without the network, and
inventing one is never acceptable.

**A queued op now carries a bounded source descriptor.** `PendingTrackRef`
(slot index + playback id + Deezer track id + Deezer ALBUM id) is exactly what
the existing resolver/ingest path already consumes, and round-trips as plain
JSON so it survives the app being killed. Replay materializes it through the
SAME `_resolveWithIngest` an online add uses — never a parallel path.

**"Unresolved" means two different things and only one is terminal.** Offline
(`lastLookupError` classifies as `transientNetwork`) → queue the intent / keep
the op queued. The catalog answered and has no such row → terminal, exactly as
before. Replay MUST use `_resolveForReplay`, not `_resolveOrThrow`: a flush that
runs while still offline (relaunch, or a reconnect edge that arrives before the
catalog is reachable) would otherwise DISCARD the user's queued add — the
regression suite pins this.

**The resolver still never throws** — it just records `lastLookupError` so the
caller can tell "offline" from "not in the catalog" using the app's one
taxonomy (`classifyPlaylistOpError`), instead of a second copy of it.

**Never send an empty id array.** `playlist_add_tracks` gates everything behind
`if v_count > 0`, so an empty array is a SILENT SUCCESS. Replay raises
localIntegrity instead.

**Launcher icon:** the adaptive foreground is generated by
`dart run tool/make_launcher_foreground.dart`, which scales the mark uniformly
so its bounding-box diagonal ends up at 60% of the final icon. It must be
authored PRE-INSET — flutter_launcher_icons writes `<inset android:inset="16%">`
into `mipmap-anydpi-v26/ic_launcher.xml`, which shrinks the drawable to 68%. If
that XML ever changes, update `kPluginInsetPerEdge` in the tool.
